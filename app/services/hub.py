...
# फाइल: hub.py (The Infinite Command Center) - सुधरा हुआ वर्शन
# ****************************************************************************

import os
import sys

# १. पाथ सुरक्षा: ताकि कोलाब हर फोल्डर को पहचान सके
sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

from app.utils.response_formatter import clean_shiv_response

# --- सभी ३२+ विभागों का महा-संगम (Safe Imports) ---

# १. कोडिंग और कोर इंजीनियरिंग (एरर फिक्स के साथ)
try:
    from app.services.agent_service import agent_manager      # कोडिंग मास्टर
except ImportError:
    try:
        from app.services.agent_service import AgentManager as agent_manager
    except ImportError:
        agent_manager = None
...
class ShivAIHub:
    ...
    def process_command(self, user_input, file_data=None):
        ...
        if any(x in cmd for x in ["code", "लिखो", "बनाओ", "बग"]):
            ...
            return clean_shiv_response("मालिक, कोडिंग एजेंट अभी लोड नहीं हो पाया है।")
        ...
        # नंबरों को हिंदी शब्दों में क्लीन करके भेजना (बिना हकलाए)
        return clean_shiv_response(response)

# शिव हब सक्रिय - रक्षक: Shri Ram nag
shiv_hub = ShivAIHub()
